...
function createMap(webmapitem) {
    ...
    mapDeferred.addCallback(function (response) {
        ...
        dojo.declare("my.StaticTiledLayer", esri.layers.TiledMapServiceLayer, {
          ...
          mycornerCoordinate : 20037508.3427892,

          constructor: function() {
            this.initialExtent = this.fullExtent = new esri.geometry.Extent({"xmin":-this.mycornerCoordinate,"ymin":-this.mycornerCoordinate,"xmax":this.mycornerCoordinate,"ymax":this.mycornerCoordinate,"spatialReference":{"wkid":102100}});
            this.spatialReference = new esri.SpatialReference({wkid:102100});

            var fixedLods = new Array();
            for (var i = 0; i < 21; i++)
            {
                fixedLods[i] = {"level" : i, "resolution" : resolution};
                resolution = resolution/2;
            }

            var resolution = this.mycornerCoordinate * 2 / 256;
            this.tileInfo = new esri.layers.TileInfo({
              "rows" : 256,
              "cols" : 256,
              "dpi" : 96,
              "origin" : {
                "x" : -this.mycornerCoordinate,
                "y" : this.mycornerCoordinate
              },
              "spatialReference" : {
              "wkid" : 102100
            }, 
		...
		// add google Map 
		 ...
		 dojo.declare("my.GoogleMapsLayer", esri.layers.DynamicMapServiceLayer, {
          ...
          getImageUrl: function(extent, width, height, callback) {
            ...
            var currentResolution = extentWidth / width;
            ...
            for (var i = 0; i < this.Lods.length; i++)
            {
              var lod = this.Lods[i];
              currentLodIndex = i;
                if (lod <= currentResolution)
                {
                    requestWidth = (extentWidth / lod);
                    requestHeight = (extentHeight / lod);
                    break;
                };
            };
          
            var mercatorCenterPoint = extent.getCenter();
            var geogCenterPoint =  esri.geometry.webMercatorToGeographic(mercatorCenterPoint);

            var params = {
              center: geogCenterPoint.getLatitude()+","+geogCenterPoint.getLongitude(),
              zoom: currentLodIndex,
              size:requestWidth.toFixed(0)+"x"+requestHeight.toFixed(0),
              maptype:"satellite",
              sensor:"false",
              key: "AIzaSyCf3mFap2LAGOgZ8dzzHXfcuzBZmz41r6s"
            };

            callback("http://maps.google.com/maps/api/staticmap?" + dojo.objectToQuery(params));
          }
        });

          map.addLayer(new my.GoogleMapsLayer());
